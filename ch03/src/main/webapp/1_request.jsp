...
<body>
	...
	<!-- 
		...
		Get 요청
		- 기본 링크 페이지 요청
		- 데이터 전송을 주소 뒤에 ? 파라미터 문자열(쿼리스트링)로 전송
		- 보안에 취약하지만 빠른 요청 처리
	...
	<h4>GET 요청</h4>
	<a href="./proc/result1.jsp?userid=a101">result1</a>
	<br>
	<!-- 주소뒤 ? 파라미터(쿼리스트링) 전송 -->
	...
	<form action="./proc/result3.jsp">
		<!--  form METHOD 속성이 없으면 GET 요청 -->
		<input type="text" name="userid" /><br> <input type="text"
			name="name" /><br> <input type="number" name="age" /><br> <input
			type="submit" name="전송하기" /><br>
	</form>
	...
	<p>
		헤더정보 :
		<%= request.getHeader("User-Agent") %><br> 프로토콜 :
		<%= request.getProtocol() %><br> 서버이름 :
		<%= request.getServerName() %><br> 서버포트 :
		<%= request.getServerPort() %><br> 요청주소 :
		<%= request.getRequestURL() %><br> 요청경로 :
		<%= request.getRequestURI() %><br> IP주소 :
	...
	<form action="./proc/register.jsp" method="get">
		<table border="1">
			...
			<tr>
				<td>생년월일</td>
				<td><input type="date" name="birty" /></td>
			</tr>
			<tr>
				<td>이메일</td>
			...
			<tr>
				...
				<td><label><input type="checkbox" name="hobby"
						...
						value="영화" />영화</label> <label><input type="checkbox" name="hobby"
						value="독서" />독서</label> <label><input type="checkbox" name="hobby"
						value="운동" />운동</label> <label><input type="checkbox" name="hobby"
						value="게임" />게임</label></td>
			</tr>
			<tr>
				<td>주소</td>
				<td><select name="addr">
